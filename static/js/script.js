...
function handleFortuneTypeClick(event) {
  ...
  if (element.classList.contains("selected")) {
    ...
    state.selectedFortuneTypes = state.selectedFortuneTypes.filter(
      (t) => t !== fortuneType
    );
  } else {
...
function displaySuccess(data) {
  ...
  if (elements.timestamp && data.timestamp) {
    ...
    elements.timestamp.textContent = `생성 시간: ${date.toLocaleString(
      "ko-KR"
    )}`;
  ...
  if (elements.resultSection) {
    ...
    console.log(
      "🖼️ 결과 섹션 클래스 추가됨:",
      elements.resultSection.classList
    );
  }
  ...
  setTimeout(() => {
    ...
    if (elements.fortuneResult) {
      console.log(
        "- 운세 텍스트:",
        elements.fortuneResult.textContent.substring(0, 50) + "..."
      );
    }
